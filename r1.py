...
#                        boundary = boundary + 1
    ...
    def attack(self,uid,enemy_id,timeneed, infantry, cavalry, catapult):
        ...
        try:
            ...
            ub.powerin=infantry
            ub.powerca=cavalry
            ub.catapult = catapult
            ub.power=infantry+cavalry+catapult
            allypower=allyhelp(uid,0,infantry+cavalry)
    ...
    def emptyLost(battle):
        ...
        returnCatapult = battle.catapult + min(returnCa, 0)
        returnIn = max(returnIn, 0)
        returnCa = max(returnCa, 0)
        returnCatapult = max(returnCatapult, 0)
    ...
    def warresult2(uid):
        ...
        for b in battleset:
            ...
            returnCatapult = b.catapult + min(returnCa, 0)
            returnIn = max(returnIn, 0)
            returnCa = max(returnCa, 0)
            returnCatapult = max(returnCatapult, 0)
...
#                    u.cae=u.cae-caesars
    ...
    def producecatapult(self,user_id,city_id,grid_id,cata_id,type):
        try:
            ...
            num_d = int(sp[3].split(',')[1])
            num_e = int(sp[4].split(',')[1])
            num_f = int(sp[5].split(',')[1])
            num_g = int(sp[6].split(',')[1])
            num_h = int(sp[7].split(',')[1])
            num_i = int(sp[8].split(',')[1])
            ...
            if int(type) == 0:
                if u.corn>=catapult[cataid][0] and u.wood>=catapult[cataid][1] and u.stone>=catapult[cataid][2]:
                    if cataid==0:
                        ...
                        num_a -= 5
                        num_b -=5
                    specialgoods = "a,"+str(num_a)+";b,"+str(num_b)+";c,"+str(num_c)+";d,"+str(num_d)+";e,"+str(num_e)+";f,"+str(num_f)+";g,"+str(num_g)+";h,"+str(num_h)+";i,"+str(num_i)+";j,"+str(num_j)+";k,"+str(num_k)+";l,"+str(num_l)
                    ...
                    p.object_id = cataid
                    read(city_id)
                    return dict(id=1, result="produce catapult suc")
                else:
                    return dict(id=0, reason="corn wood stone not enough")
            else:
                if u.cae >= catapult[cataid][6]:
                    u.cae -= catapult[cataid][6]
                    p.producttime = 1
                    p.object_id = cataid
                    read(city_id)
                    return dict(id=1)
            ...
            return dict(id=0,reason="try failed")
    @expose('json')
    def harvestcatapult(self,user_id,city_id,grid_id):
        try:
            p=DBSession.query(businessWrite).filter_by(city_id=int(city_id)).filter_by(grid_id=int(grid_id)).one()
            userid = int(user_id)
            u = checkopdata(user_id)
