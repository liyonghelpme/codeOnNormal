...
class RootController(BaseController):
    ...
    def emptyLost(battle):
        ...
        attStr += [battle.powerin, battle.powerca, attGod, battle.allypower, returnIn, returnCa, attacker.empirename, attacker.nobility*3+attack.subno,  empty.inf, empty.cav, defGod, defAlly, leftIn, leftCa, defencer.otherid, defencer.empirename, defencer.nobility]
        if attFullPow > defFullPow:
    ...
    def godbless(self,uid,godtype,caetype):
        ...
        mark=0
        warmap = DBSession.query(warMap).filter_by(userid = uid).one()
        if godtype == 4 or godtype == 5: 
            if caetype >= 0 and caetype < len(caeCost):
                buildings = DBSession.query(businessWrite).filter("businessWrite.city_id=:cid and ground_id >= 420 and ground_id <= 429 and finish = 1").params(cid=warmap.city_id).all()
                ...
                if lev <= 4:
                    if int(type) == 0:
                        if u.cae >= friendGod[lev][5]:
                            ...
                            p.finish = 0
                            p.producttime = ti
                            if p.ground_id >=425 and ground_id <= 429:
    ...
    def getPets(self, uid, cid):
        dragon = DBSession.query(Dragon.pid, Dragon.bid, businessWrite.grid_id, Dragon.state, Dragon.kind, Dragon.health, Dragon.friNum, Dragon.friList, Dragon.name, Dragon.attack, Dragon.lastFeed, Dragon.style).filter(and_(Dragon.uid == uid, businessWrite.bid==Dragon.bid)).all()
        allPets = []
        for d in dragon:
            ld = list(d)
            try:
    ...
    def getdefence(self,user_id,city_id,grid_id):
        ...
        try:
            ...
            if hour > 0 and statue.producttime > 0:
                stype = ground_id - 600
                godlev = (monstergod.ground_id - 420)%5
                defenceadd = monGodReward[stype][godlev]*hour
                user.defencepower += defenceadd
                return dict(id=0,defenceadd=defenceadd,result="get defence suc")
            else:
               ...
               if p.finish == 0:
                   ...
                   if (ti-p.producttime) >= needTime:
                       ...
                       if monster_god.producttime > 0:
                           ...
                           if (monster_god.object_id*3600+monster_god.producttime-ti)%3600 != 0:
                               p.object_id += 1
                           p.producttime = ti
                       else:
                           p.producttime = 0
